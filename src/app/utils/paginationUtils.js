...
export async function getPaginatedResults(model, searchParams, searchFields = [], projection = {}, dataFormatter = null) {
  try {
    ...
    const formattedResults = dataFormatter
      ? await Promise.all(results.map(async (record) => await dataFormatter(record)))
      : results;
