...
async function processQueue() {
  ...
  while (ecoLinkQueue.length > 0) {
    const task = ecoLinkQueue.shift();
    try {
      await createEcoLink(task);
    } catch (error) {
      console.error('Failed to process EcoLink task:', error);
  ...
  await Promise.all([
    ...
    IndividualDesignation.findOneAndUpdate(
      { ECL_EID_Individual_Id: individualId },
      {
        ...
        ECL_EID_Audit_Trail: 'Profile updated via API'
      },
      { upsert: true, new: true }
    )
  ]);

  console.log(`EcoLink created for ${individualId}`);
}