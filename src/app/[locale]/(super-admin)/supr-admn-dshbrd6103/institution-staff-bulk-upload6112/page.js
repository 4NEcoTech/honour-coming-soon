...
export default function SuperAdminStaffBulkUploadPage() {
  ...
  const handleUpload = async () => {
    ...
    if (!selectedInstitution) {
      Swal.fire({
        ...
        text: "Please select an institution to upload staff for.",
        confirmButtonText: "Okay",
      });
      return;
    }
    ...
    try {
      ...
      const institution = institutions.find(inst => inst._id === selectedInstitution);
      if (institution) {
        formData.append("institutionNum", institution.CD_Company_Num || "");
        formData.append("institutionName", institution.CD_Company_Name || "");
        formData.append("companyId", institution._id || "");
        formData.append("adminInviteeId", institution.individual_id || "");
      }
      ...
      // Use the same API endpoint as institution
  ...
  const handleDownload = () => {
    ...
    link.href = filePath;
    link.download = "staffBulkImport.xlsx";
  ...
  return (
    <div className="max-w-md mx-auto mt-10 border rounded-lg shadow-md p-6 mb-10">
      <h2 className="text-2xl font-semibold text-center text-primary mb-4">
        Super Admin - Institution Staff Bulk Upload
      </h2>
      ...
      <p className="text-sm font-semibold mb-2">
        Please upload an Excel file (10mb) of staff details here
      </p>
      <div
