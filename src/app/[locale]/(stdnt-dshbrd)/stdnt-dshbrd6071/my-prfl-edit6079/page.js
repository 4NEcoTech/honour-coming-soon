...
export default function StudentProfileEdit() {
  ...
  const populateFormsWithData = (data) => {
    ...
    personalForm.reset({
      firstName: profile?.ID_First_Name || "",
      lastName: profile?.ID_Last_Name || "",
      profileHeadline: profile?.ID_Profile_Headline || "",
      designation: profile?.ID_Individual_Designation || "",
  ...
  const handleAddLanguage = async () => {
    ...
    try {
      const languageData = {
        HCJ_JSL_Source: "01",
        HCJ_JSL_Id: profileData.profile._id,
