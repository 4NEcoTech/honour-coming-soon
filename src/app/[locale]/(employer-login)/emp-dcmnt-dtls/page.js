...
export default function CompleteProfile() {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      if (data.uploadPhoto) formData.append("photo", data.uploadPhoto);
      formData.append("individualId", session.user.id);
      formData.append("domicile", data.domicile);
      formData.append("documentType", data.documentType);
      formData.append("documentNumber", data.documentNumber);

      const response = await fetch("/api/studentdocumentdetails", {
        method: "POST",
        body: formData,
      });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className='text-primary'>
                    Admin Document Type <span className="text-destructive">*</span>
                  </FormLabel>
                  <Select
                    onValueChange={field.onChange}
                    defaultValue={field.value}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className='text-primary'>
                    Admin Document Number <span className="text-destructive">*</span>
                  </FormLabel>
                  <FormControl>
