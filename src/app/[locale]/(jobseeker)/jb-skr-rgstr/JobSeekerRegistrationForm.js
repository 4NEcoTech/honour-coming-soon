...
function JobSeekerRegistrationForm({ goToNextStep }) {
  ...
  const onSubmit = async (data) => {
    let responseData;
    try {
      const res = await fetch("/api/auth/job-seeker/signup", {
        method: "POST",
        headers: {
      ...
      toast({
        ...
        description: responseData ? t(responseData.code) : error.message,
        // error.message ||
    ...
    <div className="min-h-screen  bg-transparent sm:bg-transparent  flex justify-center items-start mt-10">
      <Card className="w-full max-w-lg p-8 rounded-lg sm:shadow-lg sm:border border-none  shadow-none sm">
        <CardHeader>
          ...
          <CardDescription>
            If you are looking for job opportunities, please proceed with
            registration.
          </CardDescription>
        </CardHeader>
        {/* Form */}
