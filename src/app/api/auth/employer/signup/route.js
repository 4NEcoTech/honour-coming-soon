 ...
 *   post:
 *     summary: Register Employer and send OTP
 *     description: |
 *       - Handles signup for Employer role (09) using email.
 *       - Validates email format.
 *       - Prevents signup if email is already registered with a different role or verified.
 *       - Sends a secure 4-digit OTP to the user's email address.
 *       - Sets JWT token and user info in secure HTTP-only cookies.
 *     tags: [Employer Registration]
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *     responses:
 *       201:
 *         description: Employer registered and OTP sent
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *       409:
 *         description: Email already registered with different role or verified
 *         content:
 *           application/json:
 *             schema:
 *               oneOf:
 ...
 *                     message:
 *                       type: string
 *                       example: "This email is already registered with role ID 06. Cannot sign up as Employer."
 *                 - properties:
 *                     code:
 *                       type: string
 *                       example: "6091_5"
...
const generateJwtToken = (user) => {
  return jwt.sign(
    { id: user._id, email: user.UT_Email, role: user.UT_User_Role },
...
export const POST = async (request) => {
  ...
  try {
    ...
    if (user) {
      if (user.UT_User_Role !== "09") {
        return NextResponse.json(
          {
            code: '6091_7',
            title: 'User Already Registered with a Different Role',
            message: `This email is already registered with role ID ${user.UT_User_Role}. Cannot sign up as Employer.`,
          },
          { status: 409 }
        );
      }

      if (user.UT_Email_Verified === '03') {
        return NextResponse.json(
          {
            code: '6091_5',
    ...
    const emailSent = await sendEmail({
      to: email,
      subject: `Welcome to HCJ - Employer Signup`,
